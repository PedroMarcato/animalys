...
public class FileUploadUtil implements Serializable {
	...
	public static String salvarArquivo(UploadedFile uploadedFile) throws IOException {
		...
		System.out.println("[server-debug][FileUploadUtil] salvarArquivo: início - nome=" + (uploadedFile!=null?uploadedFile.getFileName():"(null)") + " size=" + (uploadedFile!=null?uploadedFile.getSize():0));
		...
		if (!ALLOWED_EXTENSIONS.contains(extensao.toLowerCase())) {
			throw new IllegalArgumentException("Tipo de arquivo não permitido. Permitidos: imagens (JPG, PNG, GIF, BMP) e PDF");
		}
	...
	logger.debug("salvarArquivo: nome original='" + nomeOriginal + "', gerando nome unico='" + nomeUnico + "'");
		...
		try (InputStream input = uploadedFile.getInputstream();
		 OutputStream output = new FileOutputStream(arquivo)) {
			...
			System.out.println("[server-debug][FileUploadUtil] salvarArquivo: salvando arquivo em '" + arquivo.getAbsolutePath() + "'");
			...
			System.out.println("[server-debug][FileUploadUtil] salvarArquivo: arquivo salvo com sucesso: '" + arquivo.getAbsolutePath() + "'");
		} catch (IOException ioe) {
			logger.error("salvarArquivo: falha ao salvar arquivo: " + ioe.getMessage(), ioe);
			System.out.println("[server-debug][FileUploadUtil] salvarArquivo: falha ao salvar arquivo: " + ioe.getMessage());
			throw ioe;
		}
	...
	public static boolean removerArquivo(String nomeArquivo) {
		...
		System.out.println("[server-debug][FileUploadUtil] removerArquivo: solicitacao para remover '" + nomeArquivo + "'");
		...
		if (arquivo.exists()) {
			...
			System.out.println("[server-debug][FileUploadUtil] removerArquivo: arquivo '" + arquivo.getAbsolutePath() + "' removido? " + removed);
			return removed;
		}
		logger.warn("removerArquivo: arquivo não encontrado: '" + arquivo.getAbsolutePath() + "'");
		System.out.println("[server-debug][FileUploadUtil] removerArquivo: arquivo não encontrado: '" + arquivo.getAbsolutePath() + "'");
		return false;
	}
